...
extension BaseViewController {
    @discardableResult private func createNoInternetConnectionView() -> UIView {
        ...
        let wifiImageView = UIImageView(image: R.image.ic_wifi())
    ...
    private func createNoInternetViewCloseButton() -> UIButton {
        ...
        closeButton.backgroundColor = R.color.subTitle()
    ...
    @objc func onInternetConnectionUnavailable() {
        ...
        guard self.viewError == nil, !self.isKind(of: LoginViewController.self)
//                , !self.isKind(of: FeedScreenViewController.self)
        else {
            return
        }
...
extension BaseViewController {
    func getMessageNoData(message: String) -> NSAttributedString {
        let font = R.font.avertaStdCYRegular(size: 16)!
